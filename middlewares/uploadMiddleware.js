...
const upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    ...
    const isImage =
      imageExtensions.test(extname) && imageMimetypes.includes(file.mimetype);
    const isDocument =
      documentExtensions.test(extname) &&
    ...
    if (isImage || isDocument) {
      ...
      cb(
        new Error(
          "Only images (JPEG, JPG, PNG, GIF) and documents (CSV, XLSX, XLS) are allowed"
        )
      );
    }
  },
