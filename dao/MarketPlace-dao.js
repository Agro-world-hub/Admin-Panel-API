...
exports.checkMarketProductExistsDao = async (
  varietyId,
  displayName,
  category,
) => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT * FROM marketplaceitems WHERE category = ? AND (varietyId = ? OR displayName = ?)";
    ...
    collectionofficer.query(sql, values, (err, results) => {
      if (err) {
        ...
        const varietyExists = results.some(
          (item) => item.varietyId === varietyId,
        );
...
exports.checkMarketEditProductExistsDao = async (
  varietyId,
  displayName,
  category,
  id,
...
exports.getMarketplaceItems = (
  ...
  return new Promise((resolve, reject) => {
    ...
    let countSql = `SELECT COUNT(*) as total 
                    ...
                    LEFT JOIN collection_officer.producttypes pt ON m.productTypeId = pt.id`;
    ...
    if (productTypeId) {
      whereConditions.push("m.productTypeId = ?");
      countParams.push(productTypeId);
      dataParams.push(productTypeId);
    }
...
exports.getAllCoupenDAO = (limit, offset, status, types, searchText) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(countSql, countParms, (countErr, countResults) => {
      if (countErr) {
        console.log(countErr);

        reject(countErr);
      } else {
        collectionofficer.query(dataSql, dataParms, (dataErr, dataResults) => {
          if (dataErr) {
            console.log(dataErr);
...
exports.getProductById = async (id) => {
  return new Promise((resolve, reject) => {
    const sql = `
          ...
          JOIN plant_care.cropgroup CG ON CV.cropGroupId = CG.id
          LEFT JOIN collection_officer.producttypes PT ON MPI.productTypeId = PT.id  
          WHERE MPI.id = ?
    `;
    collectionofficer.query(sql, [id], (err, results) => {
      if (err) {
...
exports.getMarketplacePackageByIdDAO = async (id) => {
  return new Promise((resolve, reject) => {
    const sql = `
      ...
      FROM collection_officer.marketplacepackages mpp
      WHERE mpp.id = ?;
    `;
...
exports.getMarketplaceUsers = async (buyerType) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, [buyerType], (err, results) => {
      if (err) {
        reject(err);
      } else {
        resolve(results);
...
exports.deleteMarketplaceUser = async (userId) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, [userId], (err, result) => {
      if (err) {
        return reject(err);
      }
...
exports.getBannerCount = async (type) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, [type], (err, results) => {
      if (err) {
        reject(err);
      } else {
...
exports.getBannerById = async (feedbackId) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, [feedbackId], (err, results) => {
      if (err) {
        return reject(err);
      }
      resolve(results[0]);
...
exports.deleteBannerRetail = async (feedbackId, orderNumber) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(deleteSql, [feedbackId], (deleteErr, deleteResults) => {
      if (deleteErr) {
...
exports.getProductType = async () => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT typeName, isValid, shortCode, id FROM producttypes ORDER BY typeName ASC";
    collectionofficer.query(sql, (err, results) => {
      if (err) {
        return reject(err);
      }
...
exports.getAllRetailOrderDetails = (
  ...
  return new Promise((resolve, reject) => {
    let countSql = `
      ...
      FROM collection_officer.orders o 
      LEFT JOIN collection_officer.processorders po ON o.id = po.orderId
...
exports.getProductTypeByIdDao = async (id) => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT typeName, isValid, shortCode FROM producttypes WHERE id = ?";
    collectionofficer.query(sql, [id], (err, results) => {
      if (err) {
        return reject(err);
      }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const existingCharges = await new Promise((resolve, reject) => {
        const sql =
          "SELECT province, district, city, charge FROM deliverycharge";
        collectionofficer.query(sql, (err, results) => {
          if (err) return reject(err);
          resolve(results);
        });
      ...
      if (chargesToInsert.length > 0) {
        ...
        insertedCount = await new Promise((resolve, reject) => {
          collectionofficer.query(insertSql, [insertValues], (err, result) => {
            if (err) return reject(err);
      ...
      resolve({
        ...
        duplicates:
          uniqueKeyMap.size - chargesToInsert.length - chargesToUpdate.length,
...
exports.updateRetailCustomerRatingDao = (customerId, rateofCus) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, [rateofCus, customerId], (err, result) => {
      if (err) return reject(err);
...
exports.getAllMarketplaceItems = (category) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, [category], (err, results) => {
      if (err) {
        console.error(
...
exports.getCoupenDAO = async (coupenId) => {
  return new Promise((resolve, reject) => {
    const sql =
      "SELECT coupon.id, coupon.code, coupon.type, CAST(coupon.percentage AS DECIMAL(10,2)) AS percentage, coupon.status, coupon.checkLimit, CAST(coupon.priceLimit AS DECIMAL(10,2)) AS priceLimit, CAST(coupon.fixDiscount AS DECIMAL(10,2)) AS fixDiscount, coupon.startDate, coupon.endDate FROM collection_officer.coupon WHERE coupon.id = ?";

    collectionofficer.query(sql, [coupenId], (err, results) => {
      if (err) {
        reject(err);
      } else {
        resolve(
  ...
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, (err, results) => {
      if (err) {
        ...
        if (results[0].last_30_days_total === null)
          results[0].last_30_days_total = 0;
...
exports.getDefinePackageItemsBeforeDateDAO = async (
  packageId,
  providedDate,
) => {
  return new Promise((resolve, reject) => {
    const formattedDate = providedDate.includes(" ")
      ? providedDate
      : `${providedDate} 23:59:59`;
    ...
    collectionofficer.query(sql, [packageId, formattedDate], (err, results) => {
      if (err) {
        return reject(err);
      }
...
exports.getCouponByCodeDao = async (code) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, [code], (err, results) => {
      if (err) {
        reject(err);
      } else {
        resolve(results[0] || null);
...
exports.checkMarketProductExistsDaoEdit = async (
  varietyId,
  displayName,
  excludeId = null,
) => {
...
//     collectionofficer.query(sql, [isValid, modifyId, id], (err, results) => {
//       if (err) {
//         return reject(err);
//       }
//       resolve(results);
...
exports.toggleItemStatus = (id, isEnable, modifyBy) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, [isEnable, modifyBy, id], (err, result) => {
      if (err) return reject(err);
      resolve(result);
    });
  });
...
exports.disablePackagesByProductId = (productId) => {
  return new Promise((resolve, reject) => {
    ...
    collectionofficer.query(sql, [productId], (err, result) => {
      if (err) return reject(err);
      resolve(result);
    });
  });
