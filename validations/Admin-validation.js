...
exports.getFixedAssetsByCategorySchema = Joi.object({
    ...
    category: Joi.string().valid('Building and Infrastructures', 'Land', 'Machinery and Vehicles', 'Tools and Equipments').required().label('Category')
});

exports.getCurrentAssetsByCategorySchema = Joi.object({
    id: Joi.number().integer().required().label('User ID'),
    category: Joi.string().valid('Ferlizer', 'Seeds and Seedlings', 'Agro Chemicals', 'Livestock for Sale', 'Animal Feed', 'Other Consumables').required().label('Category')
});
...
exports.editAdminUserSchema = Joi.object({
    ...
    role: Joi.string().valid('SUPER_ADMIN', 'User', 'Moderator').required().label('Role')
});
