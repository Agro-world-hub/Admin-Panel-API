...
exports.getCategoriesDetailsById = async (req, res) => {
  ...
  try {
    ...
    const { id } = await ComplainCategoryValidate.IdParamsSchema.validateAsync(req.params);
...
exports.EditComplaintCategory = async (req, res) => {
  ...
  try {
    ...
    const complainCategory = await ComplainCategoryValidate.EditComplainCategorySchema.validateAsync(req.body);

    const result = await ComplainCategoryDAO.EditComplainCategoryDao(complainCategory);
    console.log(result);
    if (result.affectedRows === 0) {
      return res.json({ status: false })
    }
